...
async function fetchWithYouTubeDataApi(playlistId: string, apiKey: string): Promise<PlaylistItem[]> {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    if (!res.ok) {
      throw new Error(`YouTube Data API error: ${res.status} ${res.statusText}`);
    }
    const data = (await res.json()) as YouTubePlaylistItemsResponse;
    const items = Array.isArray(data.items) ? data.items : [];
    for (const it of items) {
      const vid = it && it.contentDetails ? it.contentDetails.videoId : undefined;
      if (typeof vid === "string" && vid.length > 0) {
        const title = typeof it?.snippet?.title === "string" ? it.snippet!.title : "Untitled";
        itemsOut.push({ title, url: `https://www.youtube.com/watch?v=${vid}&list=${playlistId}` });
      }
    }
  ...
  return itemsOut.filter((i) => (seen.has(i.url) ? false : (seen.add(i.url), true)));
}
...
function collectVideoNodesFromObject(root: unknown): Array<{ id: string; title: string }> {
    ...
    if (pvr && typeof pvr === "object") {
      ...
      if (titleObj && typeof titleObj === "object") {
        ...
        if (typeof simple === "string" && simple.length > 0) {
          ...
          const runs = (titleObj as Record<string, unknown>)["runs"] as Array<Record<string, unknown>> | undefined;
  ...
  return results.filter((r) => (seenIds.has(r.id) ? false : (seenIds.add(r.id), true)));
}

async function scrapePlaylist(videoPlaylistId: string): Promise<PlaylistItem[]> {
  ...
  if (!res.ok) {
    throw new Error(`Failed to fetch playlist page: ${res.status} ${res.statusText}`);
  }
  const html = await res.text();
  const data = tryExtractInitialDataJson(html);
  if (!data) return [];
  const nodes = collectVideoNodesFromObject(data);
  return nodes.map((n) => ({ title: n.title, url: `https://www.youtube.com/watch?v=${n.id}&list=${videoPlaylistId}` }));
}

export async function getPlaylistItems(inputUrl: string): Promise<PlaylistFetchResult> {
